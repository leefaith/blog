---
title: 面向对象编程
date: 2020-06-10 11:10:59
permalink: /pages/1bc80d
categories: 
  - 笔记
  - Javascript阮一峰
tags: 
...
- 确保拷贝后的对象，与原对象具**有同样的原型**。
- 确保拷贝后的对象，与原对象具**有同样的实例属性**。
...
有时，需要把不同的脚本合并在一个文件里面。如果一个脚本是严格模式，另一个脚本不是，它们的合并就可能出错。**严格模式的脚本在前**，则合并后的脚本都是**严格模式**；如果正常模式的脚本在前，则合并后的脚本都是正常模式。这两种情况下，合并后的结果都是**不正确**的。这时可以考虑把整个脚本文件放在**一个立即执行的匿名函数**之中。
