---
title: 标准库
date: 2020-06-10 11:10:29
permalink: /pages/a52a1a
categories: 
  - 笔记
  - Javascript阮一峰
tags: 
...
JSON 格式（JavaScript Object Notation 的缩写）是一种**用于数据交换的文本格式**，2001年由 Douglas Crockford 提出，目的是**取代繁琐笨重的 XML 格式**。

相比 XML 格式，JSON 格式符合 JavaScript 原生语法，可以由解释引擎直接处理，不用另外添加解析代码。
...
`JSON.stringify`方法用于将**一个值转为 JSON 字符串**。该字符串符合 JSON 格式，并且可以被`JSON.parse`方法还原。
